...
test.describe("Customer Weight Management", () => {
  test("Bulk Weight Data Entry for Customer", async ({ page }) => {
    ...
    await page.getByRole("searchbox", { name: "Search:" }).fill("3214567890");
    ...
    for (let i = 0; i < maxRecords; i++) {
      ...
      console.log(
        `Creating weight record ${i + 1}: ${dateString} → ${currentWeight.toFixed(1)} kg`,
      );
      ...
      await page
        ...
        .fill("Auto weight entry for testing");
      ...
      try {
        ...
        console.log(
          `⚠️ Success dialog not found for record ${i + 1}, continuing...`,
        );
      }
    ...
    console.log(
      `🎯 Bulk weight data entry completed! Created ${recordsCreated} weight records`,
    );
    console.log(
      `📊 Weight progression: 80.0kg → ${currentWeight.toFixed(1)}kg`,
    );
